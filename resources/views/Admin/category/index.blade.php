...
<div class="flex justify-between items-center mb-10">
    <a href="{{ route('categories.create') }}" class="group bg-brand-gold hover:bg-white text-black font-black py-3 px-8 rounded-2xl transition-all duration-300 uppercase tracking-widest text-xs flex items-center gap-3 shadow-lg shadow-brand-gold/20 active:scale-95">
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-white/[0.02] text-gray-400 uppercase text-[10px] font-black tracking-[0.2em] border-b border-white/5">
                <tr>
                    ...
                    <th class="px-8 py-6">Preview</th>
                    <th class="px-8 py-6">Category Details</th>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="group hover:bg-white/[0.02] transition-all duration-300">
                    ...
                    <td class="px-8 py-6">
                        @if($item->image)
                            <img src="{{ asset('storage/' . $item->image) }}" alt="Category" class="w-14 h-14 object-cover rounded-xl border border-white/10 group-hover:border-brand-gold/30 transition-all duration-500 shadow-lg">
                        @else
                            <div class="w-14 h-14 bg-white/5 rounded-xl flex items-center justify-center text-gray-600 text-[10px] font-black border border-white/10 uppercase tracking-tighter">No Image</div>
                        @endif
                    </td>
                    <td class="px-8 py-6">
                    ...
                    <td class="px-8 py-6 text-right flex items-center justify-end gap-3 mt-2">
                        <a href="{{ route('categories.edit', $item->id) }}" class="p-2.5 rounded-xl bg-white/5 border border-white/10 text-brand-gold hover:bg-brand-gold hover:text-black transition-all">
                        ...
                        <form action="{{ route('categories.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?')">
                            @csrf
                            @method('DELETE')
