...
@section('header', isset($data) ? 'Update Sub-Category' : 'Create Sub-Category')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <a href="{{ route('sub-categories.index') }}" class="text-gray-400 hover:text-gray-900 transition-colors flex items-center space-x-2 font-bold text-sm uppercase tracking-widest">
    ...
    <div class="bg-gray-50/50 rounded-3xl p-8 border border-gray-100">
        <form action="{{ isset($data) ? route('sub-categories.update', ['sub_category' => $data->id]) : route('sub-categories.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @isset($data)
                @method('PUT')
            @endisset
            ...
            <div class="space-y-6">
                <div class="mb-4">
                    <label for="category_id" class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Parent Category</label>
                    <select name="category_id" id="category_id" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-orange-500 focus:border-orange-500 block p-3 transition-all @error('category_id') border-red-500 @enderror">
                        <option value="">Select a Category</option>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ (isset($data) && $data->category_id == $category->id) || old('category_id') == $category->id ? 'selected' : '' }}>
                                {{ $category->name_en }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Representational Image</label>
