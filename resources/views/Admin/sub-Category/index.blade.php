...
@section('header', 'Sub-Category Management')

@section('content')
<div class="flex justify-between items-center mb-10">
    <a href="{{ route('sub-categories.create') }}" class="group bg-brand-gold hover:bg-white text-black font-black py-3 px-8 rounded-2xl transition-all duration-300 uppercase tracking-widest text-xs flex items-center gap-3 shadow-lg shadow-brand-gold/20 active:scale-95">
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-white/[0.02] text-gray-400 uppercase text-[10px] font-black tracking-[0.2em] border-b border-white/5">
                <tr>
                    ...
                    <th class="px-8 py-6">Parent</th>
                    <th class="px-8 py-6">Sub-Category Details</th>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="group hover:bg-white/[0.02] transition-all duration-300">
                    ...
                    <td class="px-8 py-6">
                        <span class="inline-flex items-center px-4 py-1.5 rounded-full bg-brand-gold/10 border border-brand-gold/20 text-brand-gold text-[10px] font-black uppercase tracking-widest">
                    ...
                    <td class="px-8 py-6 text-right flex items-center justify-end gap-3 mt-2">
                        <a href="{{ route('sub-categories.edit', $item->id) }}" class="p-2.5 rounded-xl bg-white/5 border border-white/10 text-brand-gold hover:bg-brand-gold hover:text-black transition-all">
                        ...
                        <form action="{{ route('sub-categories.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?')">
                            @csrf
                            @method('DELETE')
